...
const DEFAULT_MODEL: Model = { label: 'gpt-4o-mini', value: 'gpt-4o-mini' };
...
export function SettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  // 使用默認模型確保不會是 undefined
  const [model, setModel] = React.useState<Model>(models[0] || DEFAULT_MODEL);
...
export function SettingsDialog() {
  ...
  const [tempKeys, setTempKeys] = React.useState<Record<string, string>>(keys);
