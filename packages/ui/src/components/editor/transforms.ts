...
export const insertBlock = (editor: PlateEditor, type: string) => {
  editor.tf.withoutNormalizing(() => {
    ...
    if (type in insertBlockMap) {
      (insertBlockMap[type] as (editor: PlateEditor, type: string) => void)(editor, type);
    } else {
  ...
  editor.tf.withoutNormalizing(() => {
    const setEntry = (entry: NodeEntry<TElement>) => {
      ...
      if (type in setBlockMap) {
        return (setBlockMap[type] as (editor: PlateEditor, type: string, entry: NodeEntry<TElement>) => void)(editor, type, entry);
      }
      if (node.type !== type) {
