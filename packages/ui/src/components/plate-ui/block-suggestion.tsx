...
export const useResolveSuggestion = (
  ...
  const resolvedSuggestion: ResolvedSuggestion[] = useMemo(() => {
    ...
    suggestionIds.forEach((id) => {
        ...
        if (TextApi.isText(node)) {
          ...
          if (lineBreakData.type === 'insert') {
            newText += lineBreakData.isLineBreak
              ? BLOCK_SUGGESTION
              : BLOCK_SUGGESTION + (TYPE_TEXT_MAP[node.type]?.(node) || `${node.type || 'Unknown'}`);
      ...
      const nodeData = entries[0] && entries[0][0] ? api.suggestion.suggestionData(entries[0][0]) : undefined;
